...
trait Tables {
  ...
  case class MessagesRow(messageId: Int, messageFromName: String, message: String)
  ...
  implicit def GetResultMessagesRow(implicit e0: GR[Int], e1: GR[String]): GR[MessagesRow] = GR{
    prs => import prs._
    MessagesRow.tupled((<<[Int], <<[String], <<[String]))
  }
  ...
  class Messages(_tableTag: Tag) extends Table[MessagesRow](_tableTag, "MESSAGES") {
    def * = (messageId, messageFromName, message) <> (MessagesRow.tupled, MessagesRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(messageId), Rep.Some(messageFromName), Rep.Some(message)).shaped.<>({r=>import r._; _1.map(_=> MessagesRow.tupled((_1.get, _2.get, _3.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
